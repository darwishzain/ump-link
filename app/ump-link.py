...
content = tk.Frame(fmain)
content.grid(row=1, column=0)
devbar = tk.Frame(fmain, bg='#FFF')
devbar.grid(row=2, column=0, columnspan=5)

def umplink():
    kalamBtn = tk.Button(content, bg='#F0F0F0', height=5, width=10,text=line[0][2],command=lambda:command(line[0][1]), relief='flat')
    kalamBtn.grid(row=line[0][3], column=line[0][4])
    ecommBtn = tk.Button(content, height=5, width=10,text=line[1][2],command=lambda:command(line[1][1]), relief='flat')
    ecommBtn.grid(row=line[1][3], column=line[1][4])
    orBtn = tk.Button(content, height=5, width=10,text=line[2][2],command=lambda:command(line[2][1]), relief='flat')
    orBtn.grid(row=line[2][3], column=line[2][4])
    udasBtn = tk.Button(content, height=5, width=10,text=line[3][2],command=lambda:command(line[3][1]), relief='flat')
    udasBtn.grid(row=line[3][3], column=line[3][4])
    efindBtn = tk.Button(content, height=5, width=10,text=line[4][2],command=lambda:command(line[4][1]), relief='flat')
